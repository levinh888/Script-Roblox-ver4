...
ToggleShowHP.Text = "Hiển thị số máu Boss"
...
local showHP = false
local healthBillboard = nil

-- // Tìm Human Boss gần nhất
...
RunService.RenderStepped:Connect(function(dt)
	...
	-- Hiển thị số máu
	if showHP and bossTarget and bossTarget:FindFirstChild("Humanoid") and bossTarget:FindFirstChild("Head") then
		if not healthBillboard then
			...
			healthBillboard.Adornee = bossTarget.Head
			healthBillboard.AlwaysOnTop = true
			healthBillboard.Parent = bossTarget

			local label = Instance.new("TextLabel", healthBillboard)
			label.Name = "HPLabel"
		...
		local label = healthBillboard:FindFirstChild("HPLabel")
		if label then
			local hp = bossTarget.Humanoid.Health
			local maxHp = bossTarget.Humanoid.MaxHealth
			label.Text = string.format("HP: %d / %d", hp, maxHp)
		end
	elseif healthBillboard then
...
ToggleShowHP.MouseButton1Click:Connect(function()
	showHP = not showHP
	ToggleShowHP.Text = showHP and "Đang hiển thị máu Boss" or "Hiển thị số máu Boss"
end)
